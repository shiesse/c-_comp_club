...
{
    public partial class FormDecommissionPeripheral : Form
    {
        private PeripheralDevice peripheral;
        public FormDecommissionPeripheral(PeripheralDevice peripheral)
        {
            InitializeComponent();
            InitDecommisionPeripheralForm();
            this.peripheral = peripheral;
        }

        private void InitDecommisionPeripheralForm()
        {
        ...
        private void button_deccommissionPeriphExit_Click(object sender, EventArgs e)
        {
            Close();
        }

        private void button_decommissionPeriphAgree_Click(object sender, EventArgs e)
        {
            if (compare_three_strings(comboBox_repsonPerson1.SelectedItem.ToString(), 
                comboBox_repsonPerson2.SelectedItem.ToString(),
                comboBox_repsonPerson3.SelectedItem.ToString()))
            {
                string persons = comboBox_repsonPerson1.SelectedItem + "\n" + comboBox_repsonPerson2.SelectedItem + "\n" + comboBox_repsonPerson3.SelectedItem;

                WriteOffReportLogic.AddReport(DateTime.Now, TechType.peripheralDevice, peripheral.Inventory_Number, persons);

                PeripheralLogic.DecommissionPeripheral(peripheral.Id);
